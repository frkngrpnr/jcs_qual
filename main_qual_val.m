...
use_val = 0;
Nfold=6; % number of folds used to optimize parameters within labeled training set
%% 2. Data preparation
%% 2.1. Face alignment
...
ff_audio_scene_tr_val

% Step 3.2 FF(lgbptop,vggfer33fun)->ELM
ff_face_feats_tr_val

% Step 3.3: Stack predictions of 3.1 and 3.2. subsystem to RF
rf_score_fusion_tr_val

explanation_model_val % model a tree using the system from qualitative part

% We are done! Time to write predictions in submission format
if (opts.write_preds)
    prepare_submission_qual(qual_scores_final_elm_val,explanations,dimensions,qual_val_videos)
end